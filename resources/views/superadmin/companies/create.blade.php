...
@section('title', 'Tambah Perusahaan - Superadmin - DataKita')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Tambah Perusahaan</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">Tambahkan data perusahaan baru ke dalam sistem.</p>
    </div>

    <form action="{{ route('superadmin.companies.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="alamat" id="alamat" rows="4" placeholder="Masukkan alamat lengkap perusahaan (opsional)"
                          class="{{ $base }} {{ $errors->has('alamat') ? $err : $ok }}">{{ old('alamat') }}</textarea>
                @error('alamat')
                    <p class="mt-1.5 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>

...
@section('title', 'Edit Data Submission - DataKita')
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg">
        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Data Submission</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Edit data submission survei SIBSTR</p>
                </div>
            ...
            <form action="{{ route('superadmin.submissions.update', $submission) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="nama" 
                               name="nama" 
                               value="{{ old('nama', $submission->nama) }}"
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="jabatan" 
                               name="jabatan" 
                               value="{{ old('jabatan', $submission->jabatan) }}"
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                               required>
                        @error('jabatan')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="perusahaan" 
                               name="perusahaan" 
                               value="{{ old('perusahaan', $submission->perusahaan) }}"
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                               required>
                        @error('perusahaan')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea id="alamat" 
                              name="alamat" 
                              rows="3"
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">{{ old('alamat', $submission->alamat) }}</textarea>
                    @error('alamat')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>

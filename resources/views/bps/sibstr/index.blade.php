...
@section('title', 'Data Survei SIBSTR - BPS Dashboard')
@section('description', 'Daftar semua respons survei SIBSTR dari semua pengguna')

@push('styles')
<style>
/* BPS SIBSTR Data Page Styles */
.sibstr-card {
    background: white;
                        ...
                        <tr>
                            ...
                            <td>
                                <div style="display:flex; gap:0.375rem; flex-wrap:wrap;">
                                    ...
                                    @php
                                        $periodLabel = ((int) ($response->triwulan ?? 0)) === 0
                                            ? 'Tahunan ' . ($response->tahun ?? '')
                                            : 'Triwulan ' . $response->triwulan . ' ' . ($response->tahun ?? '');
                                    @endphp
                                    <button type="button" class="btn-delete"
                                            title="Hapus data survei SIBSTR periode ini"
                                            onclick="bpsConfirmDelete(
                                                '{{ route('bps.sibstr.destroy', $response->id) }}',
                                                @js($response->nama_perusahaan ?: ($response->user->name ?? 'Responden')),
                                                @js('SIBSTR ' . $periodLabel . ' — ' . ($response->user->email ?? '')),
                                                @js('SIBSTR periode ' . $periodLabel)
                                            )">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
